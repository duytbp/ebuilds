...
pkg_dir="${repo_root}/dev-util/opencode-desktop-bin"

version=${1:-}
if [[ -z "${version}" ]]; then
	tag=$(gh api repos/anomalyco/opencode/releases/latest --jq '.tag_name')
	gh api repos/anomalyco/opencode/releases/latest \
		--jq '.assets[] | select(.name == "opencode-electron-linux-amd64.deb") | .browser_download_url' \
		> /dev/null
	version=${tag#v}
fi

new_ebuild="${pkg_dir}/opencode-desktop-bin-${version}.ebuild"
if [[ -e "${new_ebuild}" ]]; then
	printf 'ebuild already exists: %s\n' "${new_ebuild}"
	exit 0
fi

latest_ebuild=$(printf '%s\n' "${pkg_dir}"/opencode-desktop-bin-*.ebuild | sort -V | tail -n1)
